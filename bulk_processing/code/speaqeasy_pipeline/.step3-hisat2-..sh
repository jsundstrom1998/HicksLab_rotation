#!/bin/bash
#SBATCH --job-name=step3-hisat2-.
#SBATCH --chdir=.
#SBATCH --output=./logs/hisat2-.%a.txt
#SBATCH --error=./logs/hisat2-.%a.txt
#SBATCH --array=1-1%15
#SBATCH --cpus-per-task=8
#SBATCH --mem=25G

##SBATCH --dependency=afterok:pipeline_setup,step2-trim-.
...
if [ -f /users/jsundstr/hicks_home/bulk_processing//trimmed_fq/${ID}_trimmed_forward_paired.fastq ] ; then
	...
	hisat2 -p 8 	-x /users/jsundstr/hicks_home/bulk_processing/hisat2_grch38/genome -1 $FP -2 $RP -U ${FU},${RU} 	-S /users/jsundstr/hicks_home/bulk_processing//HISAT2_out/${ID}_hisat_out.sam --rna-strandness RF --phred33      	2>/users/jsundstr/hicks_home/bulk_processing//HISAT2_out/align_summaries/${ID}_summary.txt
	...
	hisat2 -p 8 	-x /users/jsundstr/hicks_home/bulk_processing/hisat2_grch38/genome -U /users/jsundstr/hicks_home/bulk_processing//trimmed_fq/${ID}_trimmed.fastq 	-S /users/jsundstr/hicks_home/bulk_processing//HISAT2_out/${ID}_hisat_out.sam --rna-strandness RF --phred33 	2>/users/jsundstr/hicks_home/bulk_processing//HISAT2_out/align_summaries/${ID}_summary.txt

elif [ TRUE == "TRUE" ] ; then
	## Untrimmed, pair-end
	echo "HISAT2 alignment run on original untrimmed paired-end reads"
	hisat2 -p 8 	-x /users/jsundstr/hicks_home/bulk_processing/hisat2_grch38/genome -1 ${FILE1} -2 ${FILE2} 	-S /users/jsundstr/hicks_home/bulk_processing//HISAT2_out/${ID}_hisat_out.sam --rna-strandness RF --phred33      	2>/users/jsundstr/hicks_home/bulk_processing//HISAT2_out/align_summaries/${ID}_summary.txt

else
	## Untrimmed, single-end
	echo "HISAT2 alignment run on original untrimmed single-end reads"
	hisat2 -p 8 	-x /users/jsundstr/hicks_home/bulk_processing/hisat2_grch38/genome -U ${FILE1} 	-S /users/jsundstr/hicks_home/bulk_processing//HISAT2_out/${ID}_hisat_out.sam --rna-strandness RF --phred33 	2>/users/jsundstr/hicks_home/bulk_processing//HISAT2_out/align_summaries/${ID}_summary.txt
fi
